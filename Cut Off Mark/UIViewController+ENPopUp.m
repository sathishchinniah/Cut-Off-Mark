...
//        disButton.frame = CGRectMake((self.view.frame.size.width/2) - (buttonSize/2),(self.view.frame.size.height/2) + (self.view.frame.size.height/4) , buttonSize, buttonSize);
  ...
  disButton.frame = CGRectMake(150, 523, 40, 40);
    ...
    [disButton addTarget:self action:@selector(dismissPopUpViewController)
        forControlEvents:UIControlEventTouchUpInside];
    disButton.translatesAutoresizingMaskIntoConstraints = NO;
    
    NSLayoutConstraint *leftButtonXConstraint = [NSLayoutConstraint
                                                 constraintWithItem:disButton attribute:NSLayoutAttributeCenterX
                                                 relatedBy:NSLayoutRelationGreaterThanOrEqual toItem:overlayView attribute:
                                                 NSLayoutAttributeCenterX multiplier:1.0 constant:-7.5f];
    /* 3. Constraint to position LeftButton's Y*/
    NSLayoutConstraint *leftButtonYConstraint = [NSLayoutConstraint
                                                 constraintWithItem:disButton attribute:NSLayoutAttributeCenterY
                                                 relatedBy:NSLayoutRelationEqual toItem:overlayView attribute:
                                                 NSLayoutAttributeCenterY multiplier:1.6f constant:75.0f];

    NSLayoutConstraint * equal_w = [NSLayoutConstraint constraintWithItem:disButton
                                                               ...
                                                               multiplier:1.0
                                                                 constant:40];
    NSLayoutConstraint * equal_h = [NSLayoutConstraint constraintWithItem:disButton
...
//    /* 4. Add the constraints to button's superview*/
    [overlayView addConstraints:@[ leftButtonXConstraint,
                                 leftButtonYConstraint]];
    [disButton addConstraints:@[equal_w,equal_h]];
////    //
//    // Customize popUpView
//    
